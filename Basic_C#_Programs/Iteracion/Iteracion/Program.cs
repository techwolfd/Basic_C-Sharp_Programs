...
namespace Iteracion
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] test = { 98, 99, 85, 70, 82, 34, 90, 94 };

            for (int i = 0; i < test.Length; i++)
            {
                if (test[i] > 85)
                    Console.WriteLine("pasando puntuacion: " + test[i]);
            }

            List<int> testscor = new List<int>() { 98, 99, 81, 72, 70 };

            foreach (int score in testscor)
            {
                if(score > 85)
                {
                    Console.WriteLine("pasando puntuacion en score:   " + score);
                }
            }
